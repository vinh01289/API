...
module.exports = {
  ...
  getAllFixerM: async function (req, res) {
    ...
    try {     
      ...
      listFixer = await User.find({isChildOf: id, userType: 'Fixer', userRole: 'SuperAdmin' });       

      if (listFixer.length > 0) {
        return res.json({ status: true, listFixer: listFixer, message: 'Lay thong tin nha may sua chua thanh cong' });
      }
      else {
        return res.json({ status: false, message: 'Lay thong tin nha may sua chua that bai' });
      }
