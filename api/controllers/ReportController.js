...
module.exports = {
  ...
  getListChild: async function (req, res) {
    ...
    let d = data.id;
    let a = await User.find({ isChildOf: d, userRole: {in: ["SuperAdmin", "Owner"] } });
    ...
    try {
      ...
      const specification = actionType === 'TURN_BACK' ? {
...

        {
          ...
          dynamicPlace: {
            // displayName: `Nơi ${_getNameAction(actionType, targetUser[0].userType, true)}`,
            displayName: `${_getNameAction(actionType, targetUser[0].userType, true)}`,
            headerStyle: styles.cellGreen,
            // cellFormat: function(value, row) { // <- Renderer function, you can access also any row.property
            //   return (value === 1) ? 'Active' : 'Inactive';
            // },
  ...
  getTopExport: async function (req, res) {
    ...
    try {
      let list_isChildOf = await User.find({ isChildOf: id, userRole: {in: ['SuperAdmin', 'Owner']} });
      if (list_isChildOf.length > 0) {
        let data = await Promise.all(list_isChildOf.map(async child => {
          ...
          let historyExport = await History.find({ from: child.id, type: ["EXPORT", "SALE"],createdAt: {'>=' :endDate, '<' :startDate}  })
