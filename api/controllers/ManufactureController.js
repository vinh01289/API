...
module.exports = {
  ...
  updateBrandInformation: async function (req, res) {
    var { id,name, logo, address, phone, origin, mass, ingredient, preservation, appliedStandard, optionSafetyInstructions,
      safetyInstructions } = req.body;

    var data = await Manufacture.findOne({ id: id});
    ...
    console.log(data);
    let a = await Manufacture.updateOne({ id: data.id }).set({
      name: name,
      logo: logo,
      address: address,
      phone: phone,
  ...
  find: async function (req, res) {
    ...
    try {
      // let cylinders = []
      // if (type === '1') {
      //   cylinders = await Cylinder.find({where: credential}).populate(['manufacture','current','histories', 'exportPlace']);
      // }
      // else {
      //   cylinders = await Cylinder.find({where: credential}).populate('manufacture').populate('current').populate('histories');
      // }
  ...
  list: async function (req, res) {
    try {
      ...
      const manufactures = await Manufacture.find({owner: req.userInfo.id})
  ...
  listManufacture: async function (req, res) {
    ...
    try {
      ...
      const manufactures = await Manufacture.find({owner: parent});
      
      if (manufactures) {
        return res.json({ status: true, data: manufactures, message: "Tìm thấy danh sách thương hiệu" });
      }
      else {
        return res.json({ status: false, message: "Không tìm thấy thương hiệu nào" });
      }

    } catch (err){
      return res.json({ status: false, message: "Gặp lỗi khi tìm thương hiệu" });
    }
